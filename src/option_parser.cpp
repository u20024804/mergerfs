...
static
void
set_option(fuse_args    *args_,
           const string &option_)
{
  string option;

  option = "-o" + option_;

  fuse_opt_insert_arg(args_,1,option.c_str());
}

static
void
set_kv_option(fuse_args  *args_,
              const char *key_,
              const char *value_)
{
  std::string option;
  ...
  if(branches_.size() > 0)
    {
      ...
      set_kv_option(args_,"fsname",fsname.c_str());
    }
}
...
{
  set_kv_option(args_,"subtype","mergerfs");
}

static
int
parse_and_process(const string &value_,
                  uint64_t     *uint64_)
{
  int rv;
...
static
int
parse_and_process_errno(const string &value_,
                        int          *errno_)
{
...
static
int
parse_and_process_statfs(const string         &value_,
...
static
int
parse_and_process_statfsignore(const string               &value_,
