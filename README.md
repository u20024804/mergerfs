...
In FUSE there is an opaque "file handle" which is created by `open`, `create`, or `opendir`, passed to the kernel, and then is passed back to the FUSE userland application by the kernel. Unfortunately, the FUSE kernel driver does not always send the file handle when it theoretically should. This signifantly impacts the posibilities and behaviors of a FUSE based filesystem. To work around the fact that the kernel does not include the file handle when a client calls functions such as `fstat`, `fchmod`, `fchown`, `futimens`, etc. mergerfs does a few hacky things.
